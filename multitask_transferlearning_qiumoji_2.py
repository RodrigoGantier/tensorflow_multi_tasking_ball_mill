...
def import_data():
    #import data
    dictionary_data = np.load("/media/lab/办公/extract data Rodrigo/Rodrigo/laoshi_data/multitask_ball_mill/data.npy")
...
def sequencesFromTrainingData(batch_size, n_steps, data_n): 
    ...
    data_x = np.split(train[0][batch_init:batch_init+ventana*batch_size, :], batch_size, axis=0)
    data_y1 = np.split(train[1][batch_init:batch_init+ventana*batch_size, :], batch_size, axis=0)
    data_y2 = np.split(train[2][batch_init:batch_init+ventana*batch_size, :], batch_size, axis=0)
    data_y3 = np.split(train[3][batch_init:batch_init+ventana*batch_size, :], batch_size, axis=0)
    data_y4 = np.split(train[4][batch_init:batch_init+ventana*batch_size, :], batch_size, axis=0)
    ...
    if (batch_init+ventana*batch_size)>=train[0].shape[0]:
        batch_init = 0
...
#     saver = tf.train.Saver()
    ...
    #loss function
    loss1 = tf.sqrt(tf.reduce_mean(tf.square(tf.add(out_tk1, -y1))))
    loss2 = tf.sqrt(tf.reduce_mean(tf.square(tf.add(out_tk2, -y2))))
    loss3 = tf.sqrt(tf.reduce_mean(tf.square(tf.add(out_tk3, -y3))))
    loss4 = tf.sqrt(tf.reduce_mean(tf.square(tf.add(out_tk4, -y4))))
    #the final loss
...
#     optimizer4 = tf.train.AdamOptimizer(learning_rate=lr).minimize(loss4)
    ...
    with tf.Session() as sess:
        ...
        while error_y > 0.4:
            ...
            feed_dict ={x: batch[0], y1: batch[1], y2: batch[2], y3: batch[3], y4: batch[4], keep_prob: drop}
            ...
            if (iterations%displayer)==0:
                ...
                error_y = t_loss[0]
                print "General training loss: {0}, loss1: {1}, loss2: {2}, loss3: {3}, loss4: {4}".format(t_loss[0], t_loss[1], t_loss[2], t_loss[3], t_loss[4])
                batch = sequencesFromTestData(n_steps, data_n=0)
                feed_dict ={x: batch[0], y1: batch[1], y2: batch[2], y3: batch[3], y4: batch[4], keep_prob: 1}
                t_loss = sess.run([loss, loss1, loss2, loss3, loss4], feed_dict=feed_dict)
                print "General Test     loss: {0}, loss1: {1}, loss2: {2}, loss3: {3}, loss4: {4}".format(t_loss[0], t_loss[1], t_loss[2], t_loss[3], t_loss[4])
            
            iterations +=1
        
        batch = sequencesFromTestData(n_steps, data_n=0)
        feed_dict ={x: batch[0], y1: batch[1], y2: batch[2], y3: batch[3], y4: batch[4], keep_prob: 1}
        final_loss = sess.run(loss, feed_dict=feed_dict)
        ...
        for num_task in range(1,8):
            ...
            print "error output 1: {0}, error output 2:, {1}, error output 3: {2}, error output 4: {3}".format(mean_squared_error(out1, batch[1])**0.5, 
                                                                                                        mean_squared_error(out2, batch[2])**0.5, 
                                                                                                        mean_squared_error(out3, batch[3])**0.5, 
                                                                                                        mean_squared_error(out4, batch[4])**0.5)
            _, ax = plt.subplots(2,2)
            ax[0,0].plot(out1, "--r", batch[1], "b")
            ax[0,1].plot(out2, "--r", batch[2], "b")
            ax[1,0].plot(out3, "--r", batch[3], "b")
            ax[1,1].plot(out4, "--r", batch[4], "b")
